...
public class HomeProductDealsFragment extends Fragment implements OnMapReadyCallback, LocationsCallback, NetworkListener {
    ...
    private ArrayList<com.shopoholic.models.preferredcategorymodel.Result> categoryList;
    ...
    private String type = Constants.IntentConstant.CATEGORY;
...
//        AppUtils.getInstance().showHideViewWithAnimation(rvCategories, false);
        ...
        if (getArguments() != null && getArguments().containsKey(Constants.IntentConstant.TYPE)) {
            type = getArguments().getString(Constants.IntentConstant.TYPE);
        }

        Fragment parent = getParentFragment();
        if (parent instanceof HomeFragment && type.equals(Constants.IntentConstant.CATEGORY)) {
            ((HomeFragment)parent).setCoachMarks(tvLocation, ivShowCategory);
        }
        ...
        productsAdapter = new ProductsAdapter(mActivity, this, productList, (position, view) -> {
            ...
            if (isMapShow){
                ...
                if (productList.size() > position && position >= 0) {
                    ...
                    case R.id.iv_like:
                        if (AppUtils.getInstance().isLoggedIn(mActivity) && AppUtils.getInstance().isInternetAvailable(mActivity)) {
                            ...
                            if (fragment instanceof HomeFragment) {
                                ((HomeFragment) fragment).changeFavouriteIcon(type, product.getId(), product.getIsFavourite());
                            }
                        }
                        break;
...
//                        AppUtils.getInstance().showToast(mActivity, getString(R.string.under_development));
                        ...
                        HomeProductDealsFragment.this.startActivityForResult(intent, Constants.IntentConstant.REQUEST_PRODUCT_DEAL, options.toBundle());
                        break;
                }
            }
        });

        categoryAdapter = new CategoriesAdapter(mActivity, this, categoryList, new RecyclerCallBack() {
            @Override
            public void onClick(int position, View view) {
                switch (view.getId()) {
                    case R.id.rl_root_view:
                        if (mActivity instanceof HomeActivity && ((HomeActivity) mActivity).getFilterData() != null) {
                            if (((HomeActivity) mActivity).getFilterData().getSubCategoryDetails() != null) {
                                ((HomeActivity) mActivity).getFilterData().setCount(((HomeActivity) mActivity).getFilterData().getCount() - 1);
                            }
                            ((HomeActivity) mActivity).getFilterData().setSubCategoryDetails(null);
                            if (position != 0) {
                                if (((HomeActivity) mActivity).getFilterData().getCategoryDetails() == null) {
                                    ((HomeActivity) mActivity).getFilterData().setCount(((HomeActivity) mActivity).getFilterData().getCount() + 1);
                                }
                                ((HomeActivity) mActivity).getFilterData().setCategoryDetails(categoryList.get(position));
                            } else {
                                if (((HomeActivity) mActivity).getFilterData().getCategoryDetails() != null) {
                                    ((HomeActivity) mActivity).getFilterData().setCount(((HomeActivity) mActivity).getFilterData().getCount() - 1);
                                }
                                ((HomeActivity) mActivity).getFilterData().setCategoryDetails(null);
                            }
                        }
                        if (position > 0) {
                            selectedCategoryId = categoryList.get(position).getCatId();
                            if (filterData != null)
                                filterData.setCategoryDetails(categoryList.get(position));
                        } else {
                            selectedCategoryId = "";
                            if (filterData != null) filterData.setCategoryDetails(null);
                        }
                ...
                switch (view.getId()) {
                    ...
                    case R.id.rl_root_view:
                        if (!bannerList.get(position).getDealId().equals("0")) {
                            ...
                            Intent intent = new Intent(mActivity, ProductServiceDetailsActivity.class);
                            intent.putExtra(Constants.IntentConstant.DEAL_ID, dealId);
                            HomeProductDealsFragment.this.startActivityForResult(intent, Constants.IntentConstant.REQUEST_PRODUCT_DEAL);
                        }
                        break;
                }
            }
...
//        LocalBroadcastManager.getInstance(mActivity).unregisterReceiver(broadcastReceiver);
    ...
    private void setListeners() {
        if (mActivity instanceof HomeActivity && ((HomeActivity) mActivity).getFilterData() != null) {
            com.shopoholic.models.preferredcategorymodel.Result cat = ((HomeActivity) mActivity).getFilterData().getCategoryDetails();
            if (cat != null) {
                selectedCategoryId = cat.getCatId();
            }
        }
            ...
            case R.id.tv_location:
                if (!openPlacePicker) {
                    try {
                        ...
                        HomeProductDealsFragment.this.startActivityForResult(builder.build((HomeActivity)mActivity), Constants.IntentConstant.REQUEST_PLACE_PICKER);
...
*/
            case R.id.fl_menu_right_home:
                if (mActivity instanceof HomeActivity) {
                    ...
                    HomeProductDealsFragment.this.startActivityForResult(intent, Constants.IntentConstant.REQUEST_FILTER);
                }
                break;
            case R.id.tv_status:
...
//        params.put(Constants.NetworkConstant.COUNTRY_CODE, AppSharedPreference.getInstance().getString(mActivity, AppSharedPreference.PREF_KEY.COUNTRY_CODE));
        ...
        params.put(Constants.NetworkConstant.PARAM_DISCOUNT, type.equals(Constants.IntentConstant.CATEGORY) ? selectedPercentage : "0");
        ...
        params.put(Constants.NetworkConstant.PARAM_TYPE, Constants.NetworkConstant.CATEGORY);
        ...
        params.put(Constants.NetworkConstant.PARAM_PRODUCT_TYPE, type.equals(Constants.IntentConstant.CATEGORY) ? "1" : "2");
...
//                            int previousPosition = productList.size();
                            ...
                            AppSharedPreference.getInstance().putString(mActivity, AppSharedPreference.PREF_KEY.ADMIN_COMMISSION ,dealsResponse.getAdminCommission());
...
//            swipeRefreshLayoutLayoutParams.width = FrameLayout.LayoutParams.WRAP_CONTENT;
//            rvDealsLayoutParams.height = SwipeRefreshLayout.LayoutParams.WRAP_CONTENT;
//            rvDealsLayoutParams.width = SwipeRefreshLayout.LayoutParams.WRAP_CONTENT;
...
//            AppUtils.getInstance().animateMarker(mPositionMarker, mCurrentLocation); // Helper method for smooth
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == Constants.IntentConstant.REQUEST_PRODUCT_DEAL && resultCode == RESULT_OK && data != null && data.getExtras() != null) {
    ...
    /**
     * set category on views
     *
     * @param categories
     */
    public void setCategories(List<com.shopoholic.models.preferredcategorymodel.Result> categories) {
        com.shopoholic.models.preferredcategorymodel.Result category = new com.shopoholic.models.preferredcategorymodel.Result();
        category.setCatName(getString(R.string.all));
        ...
        if (!selectedCategoryId.equals("")) {
            for (int i = 0; i < categoryList.size(); i++) {
                if (categoryList.get(i).getCatId() != null && categoryList.get(i).getCatId().equals(selectedCategoryId)) {
                    categoryList.get(i).setSelected(true);
                    ((HomeActivity) mActivity).getFilterData().setCategoryDetails(categoryList.get(i));
                } else {
                    categoryList.get(i).setSelected(false);
                }
            }
...
//                    dealType = filterData.getTypeOfDeals() == 0 ? "" : String.valueOf(filterData.getTypeOfDeals());
            ...
            com.shopoholic.models.preferredcategorymodel.Result cat = null;
            ...
            if (selectedPercentage.equals("-1")) selectedPercentage = "";
